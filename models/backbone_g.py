"""Candidate G: quality-first dense RoPE Transformer without HRM."""
...
class PoemDenseRoPETransformer(nn.Module):
    ...
    def forward(self, token_ids: torch.Tensor, targets: torch.Tensor | None = None) -> POEMModelOutput:
        ...
        return POEMModelOutput(
            logits=logits,
            loss=loss,
            metrics={"dense_rope_layers": torch.tensor(float(len(self.layers)), device=x.device)},
        )
